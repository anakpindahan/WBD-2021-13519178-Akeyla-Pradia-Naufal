...
for(let i = 0; i < booksList["books"].length; i++){
  ...
  buyForm.classList.add("product-buy");

  let qtyInput = document.createElement("input");
  qtyInput.id = "quantity-"+(i);
  qtyInput.type = "number";
  qtyInput.min = 0;
  qtyInput.max = 100;
  qtyInput.value = 0;

  buyForm.appendChild(qtyInput);
...
function increase(i){
  ...
  qty.value++;
  let prodQty = document.getElementById("product-quantity"+(i));
  prodQty.innerText = qty.value;
}